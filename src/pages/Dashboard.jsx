...
const NAV_ITEMS = [
  ...
  { key: 'certificates', label: 'Mes certificats', icon: '◧' },
  { key: 'new', label: 'Nouveau certificat', icon: '+' },
  { key: 'api', label: 'Clés API', icon: '⌘' },
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8fefe' }}>
      ...
      <aside style={{ width: 240, background: '#0a2828', display: 'flex', flexDirection: 'column', padding: '24px 0', position: 'sticky', top: 0, height: '100vh', flexShrink: 0 }}>
        ...
        <div style={{ padding: '16px 12px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <button onClick={signOut} style={{ width: '100%', padding: '8px 12px', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, background: 'transparent', color: 'rgba(255,255,255,0.5)', fontSize: 12, cursor: 'pointer', fontFamily: 'Sora, sans-serif' }}>Déconnexion</button>
        </div>
      </aside>
  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontFamily: 'Fraunces, serif', fontSize: 30, fontWeight: 900, color: '#0a2828', marginBottom: 4 }}>Tableau de bord</h1>
        <p style={{ fontSize: 14, color: '#6a9090' }}>Bienvenue, {profile?.issuers?.name || 'Organisation'}</p>
      </div>

      {/* Plan badge */}
      <div style={{ background: pc.bg, border: `1px solid ${pc.border}`, borderRadius: 14, padding: '20px 24px', marginBottom: 28, display: 'flex', alignItems: 'center', gap: 20 }}>
        <BadgeSVG plan={plan} size={72} />
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, color: pc.text, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>Votre niveau de certification</div>
          <div style={{ fontFamily: 'Fraunces, serif', fontSize: 22, fontWeight: 900, color: pc.text }}>{plan.toUpperCase()} · VeryTrust</div>
          <div style={{ fontSize: 12, color: pc.text, opacity: 0.7, marginTop: 4 }}>Domaine : {profile?.issuers?.domain || '—'}</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 32 }}>
        {[
          { label: 'Certificats émis', value: stats.total, icon: '◧' },
          { label: 'Vérifications', value: stats.verifications, icon: '🔍' },
          { label: 'Niveau actif', value: plan.toUpperCase(), icon: '◆' },
        ].map(s => (
      ...
      {/* Recent certs */}
      <div className="card">
        <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0a2828', marginBottom: 16 }}>Certificats récents</h3>
        {certs.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '32px 0', color: '#8aadad', fontSize: 13 }}>Aucun certificat émis pour l'instant</div>
        ) : (
...
function CertificatesTab({ certs, loading }) {
  return (
    <div className="fade-in">
      <h1 style={{ fontFamily: 'Fraunces, serif', fontSize: 28, fontWeight: 900, color: '#0a2828', marginBottom: 24 }}>Mes certificats</h1>
      <div className="card">
        {loading ? <div style={{ textAlign: 'center', padding: 32 }}><div className="spinner" style={{ borderTopColor: '#0d8f8f', borderColor: '#d4eded', width: 24, height: 24, margin: '0 auto' }} /></div>
          : certs.length === 0 ? <div style={{ textAlign: 'center', padding: '32px 0', color: '#8aadad', fontSize: 13 }}>Aucun certificat émis</div>
...
function CertRow({ cert, showCopy }) {
  const pc = PLAN_COLORS['bronze']
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: '1px solid #f0fafa' }}>
      ...
      <a href={`/verify/${cert.id}`} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: '#0d8f8f', fontWeight: 600, padding: '5px 12px', border: '1px solid #a8dede', borderRadius: 6 }}>Vérifier</a>
    </div>
  )
}
...
function NewCertTab({ profile, onSuccess }) {
  ...
  return (
    <div className="fade-in">
      <h1 style={{ fontFamily: 'Fraunces, serif', fontSize: 28, fontWeight: 900, color: '#0a2828', marginBottom: 24 }}>Émettre un certificat</h1>
      <div className="card" style={{ maxWidth: 560 }}>
        ...
        <form onSubmit={handleSubmit}>
          {[
            { key: 'entityName', label: "Nom de l'entreprise", placeholder: 'Vallion SA', type: 'text' },
            { key: 'entityRef', label: 'Référence (NINEA, RC...)', placeholder: 'SN-DKR-2024-B-12345', type: 'text' },
            { key: 'fiscalYear', label: 'Exercice fiscal', placeholder: '2024', type: 'text' },
          ].map(f => (
          ...
          <div style={{ marginBottom: 24 }}>
            <label className="label">Référentiel comptable</label>
          ...
          <button className="btn-primary" style={{ width: '100%', padding: 13, fontSize: 14 }} disabled={loading}>
            {loading ? <span className="spinner" /> : 'Émettre le certificat'}
          </button>
        </form>
      </div>
    </div>
...
function ApiTab({ profile }) {
  ...
  return (
    <div className="fade-in">
      <h1 style={{ fontFamily: 'Fraunces, serif', fontSize: 28, fontWeight: 900, color: '#0a2828', marginBottom: 24 }}>Intégration API</h1>
      <div className="card" style={{ marginBottom: 20 }}>
        <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0a2828', marginBottom: 12 }}>Votre clé API</h3>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button className="btn-secondary" style={{ fontSize: 12, padding: '10px 16px', whiteSpace: 'nowrap' }} onClick={copy}>{copied ? '✓ Copié' : 'Copier'}</button>
        </div>
        <p style={{ fontSize: 11, color: '#8aadad', marginTop: 8 }}>Ne partagez jamais cette clé publiquement. Elle donne accès à l'émission de certificats en votre nom.</p>
      </div>
      <div className="card">
        <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0a2828', marginBottom: 16 }}>Exemple d'intégration</h3>
...
// cert.sealSvg  → SVG à intégrer dans le PDF
// cert.verifyUrl → URL de vérification publique`}</pre>
      </div>
    </div>
  )
}
    ...
    <div className="fade-in">
      ...
      <div className="card" style={{ maxWidth: 480 }}>
        {[
          ...
          { label: 'Organisation', value: profile?.issuers?.name },
          { label: 'Domaine', value: profile?.issuers?.domain || '—' },
          { label: 'Niveau', value: (profile?.issuers?.plan || 'bronze').toUpperCase() },
