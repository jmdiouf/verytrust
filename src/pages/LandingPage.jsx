...
const SECTORS = [
  { icon: '⚖', name: 'Experts-Comptables', desc: 'États financiers, bilans, comptes de résultat' },
  { icon: '§', name: 'Avocats', desc: 'Contrats, actes juridiques, avis de droit' },
  { icon: '◈', name: 'Experts Fiscaux', desc: 'Déclarations, conseils et optimisations' },
  { icon: '⌂', name: 'Experts Immobiliers', desc: 'Évaluations et certificats de valeur' },
  { icon: '✦', name: 'Commissaires aux Comptes', desc: "Rapports d'audit et certification légale" },
  { icon: '◉', name: 'Auditeurs Internes', desc: 'Rapports IIA, conformité et contrôle interne' },
  { icon: '♦', name: 'Notaires', desc: 'Actes notariés et certification patrimoniale' },
  { icon: '▣', name: 'Ingénieurs-Conseils', desc: 'Expertises techniques et rapports certifiés' },
]
...
export default function LandingPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'white' }}>
      ...
      {/* HERO */}
      <section style={{ padding: '80px 40px 64px', maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
        <div className="fade-in">
          ...
          <h1 style={{ fontFamily: 'Fraunces, serif', fontSize: 52, fontWeight: 900, lineHeight: 1.1, color: '#0a2828', marginBottom: 20 }}>
            Un document certifié,{' '}
            <em style={{ fontStyle: 'italic', color: '#0d8f8f' }}>zéro question.</em>
          </h1>
      ...
      {/* STATS */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderTop: '1px solid #e8f4f4', borderBottom: '1px solid #e8f4f4', background: '#f8fefe' }}>
        {[['54', t('stat1')], ['140k+', t('stat2')], ['5', t('stat3')], ['3s', t('stat4')]].map(([n, l]) => (
          <div key={l} style={{ padding: '28px 40px', borderRight: '1px solid #e8f4f4', textAlign: 'center' }}>
            <div style={{ fontFamily: 'Fraunces, serif', fontSize: 36, fontWeight: 900, color: '#0d8f8f', marginBottom: 4 }}>{n}</div>
      ...
      <section style={{ padding: '80px 40px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#0d8f8f' }} />
          <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: 3, color: '#0d8f8f', textTransform: 'uppercase' }}>Niveaux de certification</span>
        </div>
        <h2 style={{ fontFamily: 'Fraunces, serif', fontSize: 38, fontWeight: 900, color: '#0a2828', marginBottom: 48, maxWidth: 460 }}>{t('levels_title')}</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }}>
          {LEVELS.map(l => (
            <div key={l.key} style={{ borderRadius: 14, border: `1px solid ${l.border}`, padding: 24, background: 'white', transition: 'all 0.25s' }}
              ...
              <div style={{ fontFamily: 'Fraunces, serif', fontSize: 18, fontWeight: 900, color: '#0a2828', marginBottom: 8 }}>{l.sub}</div>
              <div style={{ fontSize: 12, color: '#6a9090', lineHeight: 1.6, marginBottom: 14 }}>{t(`${l.key}_desc`)}</div>
      ...
      <div style={{ background: '#f0fafa', borderTop: '1px solid #d4eded', borderBottom: '1px solid #d4eded' }}>
        <section style={{ padding: '72px 40px', maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#0d8f8f' }} />
            <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: 3, color: '#0d8f8f', textTransform: 'uppercase' }}>Professions couvertes</span>
          </div>
          <h2 style={{ fontFamily: 'Fraunces, serif', fontSize: 36, fontWeight: 900, color: '#0a2828', marginBottom: 40 }}>Un label. Toutes les professions réglementées.</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
            {SECTORS.map(s => (
              <div key={s.name} style={{ background: 'white', borderRadius: 12, padding: '20px 16px', border: '1px solid #e0f0f0', textAlign: 'center', transition: 'border-color 0.2s' }}
                onMouseEnter={e => e.currentTarget.style.borderColor = '#0d8f8f'}
                onMouseLeave={e => e.currentTarget.style.borderColor = '#e0f0f0'}>
                <div style={{ fontSize: 28, marginBottom: 10 }}>{s.icon}</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#0a2828', marginBottom: 6 }}>{s.name}</div>
                <div style={{ fontSize: 11, color: '#8aadad', lineHeight: 1.5 }}>{s.desc}</div>
              </div>
            ))}
          </div>
        </section>
      ...
      {/* CTA BAND */}
      <div style={{ background: '#0d8f8f', padding: '60px 40px', textAlign: 'center' }}>
        <h2 style={{ fontFamily: 'Fraunces, serif', fontSize: 32, fontWeight: 900, color: 'white', marginBottom: 12 }}>{t('cta_title')}</h2>
      ...
      {/* FOOTER */}
      <footer style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 40px', borderTop: '1px solid #e8f4f4', background: '#fafefe' }}>
